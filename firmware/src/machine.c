...
inline void acumulate_potentiometers(void)
{
    ...
    if (control.motor_PWM_target.samples <= ADC_AVG_VARIABLE_OVERFLOW_PROTECTION)
    { // variable overflow protection (2^32)/255 = 16843009
        control.motor_PWM_target.sum += adc.channel[MOTOR_PWM_POT].avg;
        control.motor_PWM_target.samples++;
    }
    else
        VERBOSE_MSG_MACHINE(usart_send_string("ADC_AVG_VARIABLE_OVERFLOW_PROTECTION"));

    if (control.motor_RAMP_target.samples <= ADC_AVG_VARIABLE_OVERFLOW_PROTECTION)
    { // variable overflow protection (2^32)/255 = 16843009
    ...
    if (control.MCC_POWER_target.samples <= ADC_AVG_VARIABLE_OVERFLOW_PROTECTION)
    { // variable overflow protection (2^32)/255 = 16843009
    ...
    if (control.mde_steering_wheel_position.samples <= ADC_AVG_VARIABLE_OVERFLOW_PROTECTION)
    { // variable overflow protection (2^32)/255 = 16843009
